...
def parse_serial(line):
    # Split the tag from the payload
    ...
    if tag == "[DAT]":
        data_type, data_body = payload.split(' ', 1)
        if data_type == "POS":
            coords = [float(x) for x in data_body.split(',')]
            update_blender_object(coords)
            
    elif tag == "[ERR]":
        print(f"Module Error: {payload}")
...
```cpp
void sendData(const char* type, float x, float y, float z) {
    Serial.print("[DAT] ");
    Serial.print(type);
    Serial.print(" ");
    Serial.print(x); Serial.print(",");
    Serial.print(y); Serial.print(",");
    Serial.println(z); // Ends with \n
}
...
1. **Observability:** You can watch the raw stream and know exactly what is a "coordinate" vs a "debug message."
2. **Robustness:** If the module sends a boot-up splash screen (e.g., "Welcome to Robot OS"), the Python script simply ignores it because it doesn't start with `[DAT]`.
3. **Simulation Parity:** You can manually type `[DAT] POS 1,1,1` into your terminal to test if Blender moves the object without needing the actual hardware.